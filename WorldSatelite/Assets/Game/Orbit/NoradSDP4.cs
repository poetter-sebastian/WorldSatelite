...
// NoradSDP4.cs
//
// Copyright (c) 2003-2020 Michael F. Henry
// Version 02/2020
//
	...
	internal class NoradSdp4 : NoradBase
	{
         ...
         base(orbit)
      {
         ...
         for (int pass = 1; pass <= 2; pass++)
         {
            ...
            {
               // Term not used for inclinations < 3.0 degrees
               sh = 0.0;
            }
            else
            {
         ...
         dp_ssi = dp_ssi + si;
         dp_ssl = dp_ssl + sl;
         dp_ssg = dp_ssg + sgh - ((sh == 0.0) ? 0.0 : (m_cosio / m_sinio) * sh);
         dp_ssh = dp_ssh + ((sh == 0.0) ? 0.0 : (sh / m_sinio));
